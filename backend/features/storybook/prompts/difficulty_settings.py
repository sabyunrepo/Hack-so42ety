...
def get_universal_level_prompt(level: int) -> str:
    """레벨에 해당하는 범용 프롬프트 템플릿 반환 (다국어 지원)"""
    if level not in UNIVERSAL_LEVEL_PROMPTS:
        raise ValueError(f"Invalid level: {level}. Must be 1, 2, or 3.")
    return UNIVERSAL_LEVEL_PROMPTS[level]
