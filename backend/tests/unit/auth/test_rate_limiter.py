...
class TestRateLimiterService:
    ...
    async def test_connect_success(self):
        ...
        with patch("backend.core.rate_limiter.aioredis.from_url", return_value=mock_redis):
            rate_limiter = RateLimiterService()
            await rate_limiter.connect()

            assert rate_limiter._connected is True
    ...
    async def test_connect_failure(self):
        """Redis 연결 실패 테스트"""
        with patch(
            "backend.core.rate_limiter.aioredis.from_url",
    ...
    async def test_auto_connect_if_not_connected(self):
        ...
        with patch("backend.core.rate_limiter.aioredis.from_url", return_value=mock_redis):
            ...
            # check_rate_limit이 자동으로 connect 호출
            result = await rate_limiter.check_rate_limit(
                key="auth:login:192.168.1.1",
                limit=5,
                window_seconds=60
